...
*Recommend:* Clone the janus-ikg git repo and replace the files in the respective folders of the janus-graph with the files in the repo
- the file at //scripts/empty-sample.groovy// binds the traversal objects to the global variables so they are available when someone connects to the gremlin server remotely. 
  - This is one file that should not be replaced before we actually configured the custom graph and added the schema.
    - use the file scripts/ikg-setup.groovy to configure and setup the graph with schema
...
* Advantages
** Advantages of using graph databases for data management
- Graph databases have superior performance when querying connected data (Big or Small). 
- Graph databases can be updated in real time and support queries simultaneously.
- A draw back of existing system is that any query will touch the majority of the system, while a graph database will only touch the relevant data.
- It's easy to find hubs of influence in a graph database. For example: we can use page rank and find concentration points
** Advantages for visulaizations
- we can integrate the graph db with 3rd party tools to make life easier for an analyst.
- We can give the same to the sales people to generate custom graphs
...
| gremlin                                                           | RDF                                                                                      |
|-------------------------------------------------------------------+------------------------------------------------------------------------------------------|
| gremlin is a property graph model                                 | resource description framework. the data model is a little different (not going into it) |
| from apache                                                       | from w3c                                                                                 |
| real world and much better suited for things like OLTP queries    | very well suited for datamining                                                          |
| functional and traversive language (also can do pattern matching) | pattern matching language                                                                |
| allows you to design databases and define processes               |                                                                                          |
|                                                                   |                                                                                          |

** what is graph  and how to represent it in apache tinker-pop?
- entities are which are real type (vertices)
- Add label to entities
- graph.addVertex("movie") to add vertex. graph is an object and addVertex is an opensource api
- edges are relationships betweeen vertices and are always directed and always have labels
